...
class QNNRegressor:
    ...
    def fit(self, x, y):
        ...
        theta_init = [self.u_out.get_parameter(ind) for ind in range(parameter_count)]
        result = minimize(
            QNNRegressor._cost_func, theta_init, args=(x, y, self), method="Nelder-Mead"
        )
        theta_opt = result.x
    ...
    @staticmethod
    def _cost_func(theta, x_train, y_train, regressor: QNNRegressor):
        regressor._update_u_out(theta)
        y_predict = [regressor.predict(x) for x in x_train]
        if regressor.cost == "mse":
            return ((y_predict - y_train) ** 2).mean()
        else:
            raise NotImplementedError(
                f"Cost function {regressor.cost} is not implemented yet."
            )
