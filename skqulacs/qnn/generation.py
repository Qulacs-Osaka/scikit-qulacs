...
class QNNGeneretor(QNN):
    ...
    def __init__(
        ...
        sameを指定した場合、完全一致で判定を行う。

        gaussの場合、|x-y|=4√σ を超える場合(つまりkが0.000335以下)は打ち切る。
    ...
    def fit(self, train_data, maxiter: Optional[int] = None):
        """
        :param train_data: 入力したい値を入力
        data_typeによってList[int]を入力するべきか、List[string]かになる。
        
        :param maxiter: scipy.optimize.minimizeのイテレーション回数
        :return: 学習後のロス関数の値
        :return: 学習後のパラメータthetaの値
        """
        y_scaled = self.do_y_scale(train_data)
        ...
        if self.solver == "Adam":
            ...
            loss = self.cost_func(theta_now, train_data)
            theta_opt = theta_now
        else:
    ...
    def predict(self):
        ...
        y_pred_in = self._predict_inner()
        y_pred_conj=y_pred_in.conjugate()

        data_per=y_pred_in*y_pred_conj #2乗の値
        
        if self.n_qubit != self. Fqubit: #いくつかのビットを捨てる
            data_per.reshape(2**(self.n_qubit-self.Fqubit),2**self.Fqubit)
            data_per=np.sum(data_per,axis=0)
    ...
    def cost_func(self, theta,train_scaled):
        ...
        elif self.data_type == "exp_hamming":
            self.circuit.update_parameters(theta)
